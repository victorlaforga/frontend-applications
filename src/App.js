...
`
        ...
        const runQuerySchoeisel = (url, querySchoeisel) => {
          fetch(url+"?query="+ encodeURIComponent(querySchoeisel) +"&format=json")
          .then(res => res.json())
          .then(json => {
        ...
        const runQueryBovenkant = (url, queryBovenkant) => {
          fetch(url+"?query="+ encodeURIComponent(queryBovenkant) +"&format=json")
          .then(res => res.json())
          .then(json => {
        ...
        const runQueryOnderkant = (url, queryOnderkant) => {
          // Call the url with the query attached, output data
          fetch(url+"?query="+ encodeURIComponent(queryOnderkant) +"&format=json")
          .then(res => res.json())
          .then(json => {
  ...
  return (
    <div className="App">
      ...
      {this.state.data.length > 0 ? <Hoofdbedekking info={this.state.data} /> : 'Loading...'}
      {this.state.dataBovenkant.length > 0 ? <Bovenkant info={this.state.dataBovenkant} /> : 'Loading...'}
      {this.state.dataOnderkant.length > 0 ? <Onderkant info={this.state.dataOnderkant} /> : 'Loading...'}
      {this.state.dataSchoeisel.length > 0 ? <Schoeisel info={this.state.dataSchoeisel} /> : 'Loading...'}
    </div>
